...
_irq_enable: /* Todo lo de abajo puede reemplazarse con CPSIE I */
    DSB
    MRS R0, CPSR
    BIC R0, #0x80
    MSR CPSR, R0
...
.align 4
_irq_disable: /* Todo lo de abajo puede reemplazarse con CPSID I */
    DSB
    MRS R0, CPSR
    ORR R0, #0x40
    MSR CPSR, R0
...
/* Carga la dirección base en la Public RAM de la tabla de ISR */
...
/* utilizamos la instrucción WFE (Wait for Event) para generarnos un "Magic Breakpoint" propio y poder debuggear el código */
.align 4
_BREAKPOINT_DEBUG:
    WFE
    BX LR
...
/* funciones de escritura y lecutra de 8, 16 y 32 bits. Para las funciones _WRITE_X, desde C es: _WRITE_X(DIRECCION, VALOR) */
.align 4
_WRITE_8:
